...
\rowcolors{2}{gray!15}{white}
    ...
    \begin{code}
        extern typedef struct \{
           \ind method \p{name}(\p{$type_1$ $value_1$}) -> \p{out_type};
        \} \p{sample}_interface_t;
    \end{code}

    Obligatory definition in C \cod{.h} file:
    \begin{code}
        SIM_INTERFACE(\p{sample}) \{  \cmtd{essentially 'typedef struct' also}
            \ind \p{out_type} (*\p{name})(conf_object_t *obj, \p{$type_1$ $value_1$});
        \}
    \end{code}
