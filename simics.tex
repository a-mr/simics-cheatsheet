...
\subsection{Glossary \& Documentation}
    \settowidth{\MyLen}{\textit{organization.}}
...
\subsection{Glossary \& Documentation}
    \settowidth{\MyLen}{\texttt{dml-1.4-reference-manual}}
...
Assignments are equivalent to casts and hence can truncate:
        ...
        \begin{code}
            local uint16 $i$ = 0x7fff; local int8 $j$ = 2;  \cmtd{let us sum them:}
            \vspace{0.1cm}
            local int16 \tikzmark{b} $x$ = $\overbrace{\ind i + j; \ind }^{\text{calculates as int64 $\rightarrow$ 0x8001} \tikzmark{a}}$ \DrawBottomReverse{black}{truncates to int16} \cmtd{finally results in $x == 1$}
            \vspace{0.3cm}
        \end{code}
...
in comparisons int64 vs. uint64 the operands are converted to int64!
        ...
        \begin{code}
            ...
            \end{tabular}
            u == cast(-1, uint64) \cmtd{true. As comparison is between two uint64}
...
\rowcolors{2}{gray!15}{white}
    ...
    \begin{tabulary}{\linewidth}{|llll|}
        \hline
        DML construct & checkpointed & address mapped & fields \\
        \hline
        session & - & - & - \\
        saved & + & - & - \\
        attributes & + & - & - \\
    ...
    \begin{code}
        extern typedef struct \{
           \ind method \p{name}(conf_object_t *obj, \p{$type_1$ $value_1$})
           \ind \ind -> \p{out_type};
        \} \p{sample}_interface_t;
    \end{code}

    Obligatory definition in C \cod{.h} file:
    \begin{code}
        SIM_INTERFACE(\p{sample}) \{  \cmtd{also typedef struct}
            \p{out_type} (*\p{name})(conf_object_t *obj, \p{$type_1$ $value_1$});
        \}
    \end{code}

    \subsubsection{\cod{connect}s and \cod{implement}s}
...
\subsection{Attribute values}
    ...
    %There are only these 7 types of \cod{attr\_value\_t}:
    ...
    \p{T} is one of:
        ...
        \begin{tabulary}{\linewidth}{|LLL|}
            \hline
            \p{T}         & type spec & DML/C type \\
            \hline
            uint64, int64 & i & uint64, int64 \\
            boolean       & b & bool \\
            floating      & f & double \\
            string        & s & char* \\
            object        & o & conf\_object\_t \\
            list          & [$x_1..x_n$] & fixed-width tuple with types $x_1..x_n$ \\
            list          & [$x*$] & arbitrary-width array of $x$ \\
            list          & [$x+$] & non-empty arbitrary-width array
                                     of $x$ \\
            list          & $[x\{m:n\}]$ & array with $m ≤ size ≤ n$ \\
            list          & $[x\{n\}]$ & fixed-width tuple with $n$
                                         elements of $x$ \\
            dict          & D & array of attr\_dict\_pair\_t \\
            data          & d & uint8* \\
            nil           & n & void or $x$* \\
    ...
    types there are pre-defined templates \cod{uint64\_attr},
    \cod{int64\_attr}, \cod{bool\_attr}, \cod{double\_attr}.
...
\subsection{Attribute initialization}
    %\begin{tabulary}{\linewidth}{|LLL|}
        ...
        \hline
        Execution stage & SIM\_object\_is\_configured(obj) &
            SIM\_is\_restoring\_state(\,) \\
        \hline
...
\subsection{Hash tables}
        ...
        \begin{code}
            ...
            \vshort{...}
            \kw{local} ht\_str\_table\_t tab;  \cmtd{str — string (aka const char*) keys.}
            ht\_init\_str\_table(\kw{\&}tab, \cmtcommon{/*keys\_owned*/} true);
            local double \kw{*}value = new double;  \kw{*}value = 10.0;
            ht_insert_str(\kw{\&}table, "key", cast(value, void\kw{*}));
            local double \kw{*}get_back = cast(ht_lookup_str(\kw{\&}tab, "key"), double\kw{*});
            assert \kw{*}get_back == 10.0
        \end{code}
        There are also tables for \cod{int} keys or general (\cod{common}) keys.
...
%\begin{tikzpicture}
%
%    \node[block, fill=lightgray] (a) {\p{block\_a} (DML device)};
...
\begin{tikzpicture}
    \node[block, fill=lightgray] (b) {\p{block\_b} (DML device)};
...
\begin{tikzpicture}
    \node[block, fill=lightgray] (x) {\p{big\_monster}\_comp (Python class)};
    \node[block, below=of x, fill=lightgray,xshift=-1cm] (c)
        {\p{block\_c} (DML device)};
    \node[block, right=of c, fill=lightgray] (d) {\p{block\_d} (DML device)};
