...
\subsection{Syntax}
    ...
    \begin{code}
        ...
        \kw{log} \p{log-type}, \p{level}, \p{groups}: \p{"format-string"}, $arg_1$, ..., $arg_N$;
    \end{code}
    ...
    \begin{code}
        ...
        \local \ty{$type$ *} x = \kw{new} \ty{$type$};  \cmtd{e.g. for int array:}
    ...
    \begin{code}
        method \p{name}($inputType_1$ $arg_1$) -> ($outputType_1$, $outputType_2$) \{
            \vshort{\ind ...}
            \ind local $outputType_1$ $var_1$ = ...; local $outputType_2$ $var_2$ = ...;
            \ind return $(var_1, var_2)$;
        \}
        \cmt{calling this method:}
