...
\begin{code}
            local uint16 $i$ = 0x7fff; local int8 $j$ = 2;
            \vspace{0.1cm}
            local int16 \tikzmark{b} $x$ = $\overbrace{\ind i + j \ind \tikzmark{a}}^{\text{calculated as int64(0x8001)}}$; \DrawBottomReverse{black}{truncated to int16} \cmtd{results in $x == 1$}
            \vspace{0.4cm}
\end{code}


\subsubsection{Comparisons act as uint64 or int64: $==$, $<$, $<=$}
In comparison int64 - uint64 the operands are cast to int64.
        ...
        \begin{code}
            ...
            \begin{tabular}{@{}ll}
                u == -1 & \cmtd{FALSE! Equivalent to int64(u) == int64(-1),} \\
                        & \cmtd{where the upper bit of u is cropped ($\mathbf{2^{63}-1}$).}
            \end{tabular}
...
\subsection{Attribute initialization}
    %\begin{tabulary}{\linewidth}{|LLL|}
        ...
        Manual attribute assignment (hot plug) & + & - \\
        \hline
    \end{tabulary}
